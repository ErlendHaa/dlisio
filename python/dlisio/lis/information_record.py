...
class InformationRecord():
    ...
    A Information Record can contain:
    ...
    Either the record is structured as a table or not each individual entry, or
    listing, is represented by a LIS Component Block (CB)
    ...
    *Information Records*. The table in ``figure 3.9: Logical Record Types``
    ...
    def table_name(self):
        ...
        return self.attic.components[0]

    def table(self, simple=False, fill=-999.25):
