...
noexcept (false) {
    ...
    switch (err) {
        ...
        case DLIS_BAD_SIZE: {
            if (trim - segment_size != dl::SIZEOF_LRSH) {
                const auto msg =
...
record& extract(stream& file, long long tell, long long bytes, record& rec,
    ...
    while (true) {
        char buffer[ dl::SIZEOF_LRSH ];
        auto nread = file.read( buffer, dl::SIZEOF_LRSH );
        if ( nread < dl::SIZEOF_LRSH )
...
noexcept (false) {
    ...
    while (true) {
        try {
            read = file.read(buffer, dl::SIZEOF_LRSH);
